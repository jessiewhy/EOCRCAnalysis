...
data_inactive_geo_race2 <- left_join(fifty_states,data_physact2_2019_inactive_race, by='state_desc')
data_inactive_geo_race2 <- data_inactive_geo_race2 %>% drop_na()
head(data_inactive_geo_race)
```
...
```{r}
ggplot() + 
  geom_polygon(data=data_inactive_geo_race2, aes(x=long, y=lat, group = group, fill=percent), color='white') +
  scale_size(name="") + 
