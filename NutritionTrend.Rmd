...
```{r}
data_nutrition_geo_age_wwv <- data_nutrition_geo_age %>%
  filter(state_cd %in% c('WI', 'WY', 'WV', 'VA'))
p <- ggplot() + 
  ...
  labs(fill = 'Percent %')
p + geom_polygon(data=data_nutrition_geo_age_wwv, aes(x=long, y=lat, group = group, fill=percent, color=state_desc), size=1) + 
  labs(color = 'State')
```
...
data_nutrition_geo_sex_wwv <- data_nutrition_geo_sex %>%
  filter(state_cd %in% c('WY', 'NV'))
p <- ggplot() + 
  ...
  # ggtitle('Inactive Adults - Race') + 
  labs(fill = 'Percent %')
p + geom_polygon(data=data_nutrition_geo_sex_wwv, aes(x=long, y=lat, group = group, fill=percent, color=state_desc), size=1) +
  labs(color = 'State')
```
...
# state trends by age and sex

age
18-24: nevada, georgia, new mexico, oklahoma, wyoming
25 - 34: nevada, mississippi, arkansas, alabama, kentucky
35 - 44: oklahoma, louisiana, west virginia, kentucky, north dakota
45 - 54: mississippi, oklahoma, louisiana, west virgina, kentucky

```{r}
data_nutrition_geo_age %>%
  select(state_desc, age, percent) %>%
  group_by(state_desc, age, percent) %>%
  arrange(desc(percent)) %>%
  filter(age == '45 - 54') %>%
  distinct()
```
```{r}
data_nutrition_geo_sex %>%
  select(state_desc, age, percent) %>%
  group_by(state_desc, age, percent) %>%
  arrange(desc(percent)) %>%
  filter(age == '45 - 54') %>%
